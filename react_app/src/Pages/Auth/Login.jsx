...
import LoginEffects from "../Components/Effects/Auth/LoginEffects";
...
export default function Login(){
  ...
  // stateと変数の定義
  const{apiURL,fromURL,navigate,userName,setUserName,passWord,setPassWord,userNameRef,passWordRef,checkValue,setCheckValue,token,setToken,existedUser,setExistedUser,error,setError,errorCss,setErrorCss,envType,setEnvType}=LoginDefinition()


  // 動的なハンドルの定義
  const {onUserNameChange,onPassWordChange,onCheckClick,onLoginBtnClick}=LoginAction(setUserName,userNameRef,setPassWord,passWordRef,checkValue,setCheckValue,token,userName,passWord,navigate,setError,error,PasswordStoreToStorage,envType)
  ...
  DefaultSetting(apiURL,fromURL,setToken,setExistedUser,setEnvType,navigate)
  ...
  return(
    <HelmetProvider>
      ...
      <div className="my-10 base_frame border-2 border-black rounded-md text-center py-5 bg-green-50 max-w-[700px]">
        <h2  className="base_h w-[90%] min-w-[280px] text-lg mb-5 ">ユーザー名とパスワードを<br className="sm:hidden"/>入力してください</h2>
        ...
        <p className='base_link_p min-w-[280px]'>新規登録は<Link className='base_link' to="/auth/register">こちら</Link></p>
      
        <p className='base_link_p min-w-[280px]'>登録データ変更は<Link className='base_link' to="/auth/data_change">こちら</Link></p>
